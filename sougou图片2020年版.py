#/html/body/div[1]/div/div/div[2]/div/ul/li[1]/div/a[1]/img

from selenium import webdriver #从自动化库里导入这个模块
from selenium.webdriver.support.ui import WebDriverWait#从模块导入显示等待（隐式，强制）
from selenium.webdriver.support import expected_conditions as EC#导入模块（）可以对网页上元素，弹窗是否存在，可点击等等进行判断，一般用于断言或与WebDriverWait配合使用。
...
def start_spiders():#定位是关键，找到输入框等等，选择元素，通过特征
    # 找到输入框id
    query = browser.find_element_by_name('query')#和下方定位同理
    keyword = input('输入：')
    query.send_keys(keyword)#query是一个element对象，用send_keys方法输入字符串
    # 找到搜索按钮
    swz = browser.find_element_by_class_name('search-btn')#找到搜索按钮，找到这个swz标签（classname是这个，如果name或者id唯一用name，id找也行（找特征））
    swz.click()
    # 找到登陆按钮并点击
    ...
    while True:
        ...
        lis = browser.find_elements_by_xpath('/html/body/div[1]/div/div/div[2]/div/ul/li')#从根目录找到class为。。的ul元素
        ...
        for li in lis:
            ...
            href = li.find_element_by_xpath('./div/a[1]/img').get_attribute('src')
            # # 作者
